...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=https%3A%2F%2Fgithub.com%2Fjunction-api%2Fjunction-go)

The Junction Go library provides convenient access to the Junction APIs from Go.
...
A full reference for this library is available [here](https://github.com/junction-api/junction-go/blob/HEAD/./reference.md).
...
```go
response, err := client.Link.BulkImport(...)
if err != nil {
...
```go
response, err := client.Link.WithRawResponse.BulkImport(...)
if err != nil {
    return err
}
...
response, err := client.Link.BulkImport(ctx, ...)
```
...
response, err := client.Link.BulkImport(ctx, request, ...)
```
