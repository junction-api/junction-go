// Code generated by Fern. DO NOT EDIT.

package labtests

import (
	bytes "bytes"
	context "context"
	io "io"
	http "net/http"

	junctiongo "github.com/junction-api/junction-go/v2"
	core "github.com/junction-api/junction-go/v2/core"
	internal "github.com/junction-api/junction-go/v2/internal"
	option "github.com/junction-api/junction-go/v2/option"
)

type RawClient struct {
	baseURL string
	caller  *internal.Caller
	options *core.RequestOptions
}

func NewRawClient(options *core.RequestOptions) *RawClient {
	return &RawClient{
		options: options,
		baseURL: options.BaseURL,
		caller: internal.NewCaller(
			&internal.CallerParams{
				Client:         options.HTTPClient,
				MaxAttempts:    options.MaxAttempts,
				DisableRetries: options.DisableRetries,
			},
		),
	}
}

func (r *RawClient) Get(
	ctx context.Context,
	request *junctiongo.GetLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[[]*junctiongo.ClientFacingLabTest], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/lab_tests"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response []*junctiongo.ClientFacingLabTest
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[[]*junctiongo.ClientFacingLabTest]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) Create(
	ctx context.Context,
	request *junctiongo.CreateLabTestRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingLabTest], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/lab_tests"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingLabTest
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingLabTest]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetById(
	ctx context.Context,
	request *junctiongo.GetByIdLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingLabTest], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/lab_tests/%v",
		request.LabTestId,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.ClientFacingLabTest
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingLabTest]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) UpdateLabTest(
	ctx context.Context,
	request *junctiongo.UpdateLabTestRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingLabTest], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/lab_tests/%v",
		request.LabTestId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingLabTest
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPatch,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingLabTest]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetMarkers(
	ctx context.Context,
	request *junctiongo.GetMarkersLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.GetMarkersResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/lab_tests/markers"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.GetMarkersResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.GetMarkersResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetMarkersForOrderSet(
	ctx context.Context,
	request *junctiongo.GetMarkersForOrderSetLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.GetMarkersResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/lab_tests/list_order_set_markers"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.GetMarkersResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.GetMarkersResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetMarkersForLabTest(
	ctx context.Context,
	request *junctiongo.GetMarkersForLabTestLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.GetMarkersResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/lab_tests/%v/markers",
		request.LabTestId,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.GetMarkersResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.GetMarkersResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetMarkersByLabAndProviderId(
	ctx context.Context,
	request *junctiongo.GetMarkersByLabAndProviderIdLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingMarker], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/lab_tests/%v/markers/%v",
		request.ProviderId,
		request.LabId,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.ClientFacingMarker
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingMarker]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetLabs(
	ctx context.Context,
	opts ...option.RequestOption,
) (*core.Response[[]*junctiongo.ClientFacingLab], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/lab_tests/labs"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response []*junctiongo.ClientFacingLab
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[[]*junctiongo.ClientFacingLab]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) EstimateOrderSetPricing(
	ctx context.Context,
	request *junctiongo.EstimateOrderSetPricingBody,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.EstimateOrderSetPricingResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/lab_test/estimate_order_set_pricing"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.EstimateOrderSetPricingResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.EstimateOrderSetPricingResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetPaginated(
	ctx context.Context,
	request *junctiongo.GetPaginatedLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.LabTestResourcesResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/lab_test"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.LabTestResourcesResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.LabTestResourcesResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetLabTestCollectionInstructionPdf(
	ctx context.Context,
	request *junctiongo.GetLabTestCollectionInstructionPdfLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[io.Reader], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/lab_test/%v/collection_instruction_pdf",
		request.LabTestId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	response := bytes.NewBuffer(nil)
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[io.Reader]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetOrders(
	ctx context.Context,
	request *junctiongo.GetOrdersLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.GetOrdersResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/orders"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.GetOrdersResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.GetOrdersResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetPhlebotomyAppointmentAvailability(
	ctx context.Context,
	request *junctiongo.GetPhlebotomyAppointmentAvailabilityLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.AppointmentAvailabilitySlots], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/order/phlebotomy/appointment/availability"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.AppointmentAvailabilitySlots
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.AppointmentAvailabilitySlots]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) BookPhlebotomyAppointment(
	ctx context.Context,
	request *junctiongo.BookPhlebotomyAppointmentLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/phlebotomy/appointment/book",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) RequestPhlebotomyAppointment(
	ctx context.Context,
	request *junctiongo.RequestAppointmentRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/phlebotomy/appointment/request",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) ReschedulePhlebotomyAppointment(
	ctx context.Context,
	request *junctiongo.ReschedulePhlebotomyAppointmentLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/phlebotomy/appointment/reschedule",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPatch,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) CancelPhlebotomyAppointment(
	ctx context.Context,
	request *junctiongo.ApiApiV1EndpointsVitalApiLabTestingOrdersHelpersAppointmentCancelRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/phlebotomy/appointment/cancel",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPatch,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetPhlebotomyAppointmentCancellationReason(
	ctx context.Context,
	opts ...option.RequestOption,
) (*core.Response[[]*junctiongo.ClientFacingAppointmentCancellationReason], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/order/phlebotomy/appointment/cancellation-reasons"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response []*junctiongo.ClientFacingAppointmentCancellationReason
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[[]*junctiongo.ClientFacingAppointmentCancellationReason]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetPhlebotomyAppointment(
	ctx context.Context,
	request *junctiongo.GetPhlebotomyAppointmentLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/phlebotomy/appointment",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetAreaInfo(
	ctx context.Context,
	request *junctiongo.GetAreaInfoLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.AreaInfo], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/order/area/info"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.AreaInfo
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.AreaInfo]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetPscInfo(
	ctx context.Context,
	request *junctiongo.GetPscInfoLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.PscInfo], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/order/psc/info"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.PscInfo
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.PscInfo]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetOrderPscInfo(
	ctx context.Context,
	request *junctiongo.GetOrderPscInfoLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.PscInfo], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/psc/info",
		request.OrderId,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.PscInfo
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.PscInfo]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetResultPdf(
	ctx context.Context,
	request *junctiongo.GetResultPdfLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[io.Reader], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/result/pdf",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	response := bytes.NewBuffer(nil)
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[io.Reader]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetResultMetadata(
	ctx context.Context,
	request *junctiongo.GetResultMetadataLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.LabResultsMetadata], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/result/metadata",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.LabResultsMetadata
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.LabResultsMetadata]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetResultRaw(
	ctx context.Context,
	request *junctiongo.GetResultRawLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.LabResultsRaw], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/result",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.LabResultsRaw
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.LabResultsRaw]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetLabelsPdf(
	ctx context.Context,
	request *junctiongo.GetLabelsPdfLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[io.Reader], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/labels/pdf",
		request.OrderId,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	response := bytes.NewBuffer(nil)
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[io.Reader]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetPscAppointmentAvailability(
	ctx context.Context,
	request *junctiongo.GetPscAppointmentAvailabilityLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.AppointmentAvailabilitySlots], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/order/psc/appointment/availability"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.AppointmentAvailabilitySlots
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.AppointmentAvailabilitySlots]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) BookPscAppointment(
	ctx context.Context,
	request *junctiongo.BookPscAppointmentLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/psc/appointment/book",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	if request.IdempotencyKey != nil {
		headers.Add("x-idempotency-key", *request.IdempotencyKey)
	}
	if request.IdempotencyError != nil {
		headers.Add("x-idempotency-error", *request.IdempotencyError)
	}
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) ReschedulePscAppointment(
	ctx context.Context,
	request *junctiongo.ReschedulePscAppointmentLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/psc/appointment/reschedule",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPatch,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) CancelPscAppointment(
	ctx context.Context,
	request *junctiongo.VitalCoreClientsLabTestGetlabsSchemaAppointmentCancelRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/psc/appointment/cancel",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPatch,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetPscAppointmentCancellationReason(
	ctx context.Context,
	opts ...option.RequestOption,
) (*core.Response[[]*junctiongo.ClientFacingAppointmentCancellationReason], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/order/psc/appointment/cancellation-reasons"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response []*junctiongo.ClientFacingAppointmentCancellationReason
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[[]*junctiongo.ClientFacingAppointmentCancellationReason]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetPscAppointment(
	ctx context.Context,
	request *junctiongo.GetPscAppointmentLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingAppointment], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/psc/appointment",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.ClientFacingAppointment
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingAppointment]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetOrderTracking(
	ctx context.Context,
	request *junctiongo.GetOrderTrackingLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.OrderTracking], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/tracking",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.OrderTracking
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.OrderTracking]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetOrderCollectionInstructionPdf(
	ctx context.Context,
	request *junctiongo.GetOrderCollectionInstructionPdfLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[io.Reader], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/collection_instruction_pdf",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	response := bytes.NewBuffer(nil)
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[io.Reader]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetOrderRequistionPdf(
	ctx context.Context,
	request *junctiongo.GetOrderRequistionPdfLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[io.Reader], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/requisition/pdf",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	response := bytes.NewBuffer(nil)
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[io.Reader]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetOrderAbnPdf(
	ctx context.Context,
	request *junctiongo.GetOrderAbnPdfLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[io.Reader], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/abn_pdf",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	response := bytes.NewBuffer(nil)
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[io.Reader]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetOrder(
	ctx context.Context,
	request *junctiongo.GetOrderLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingOrder], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.ClientFacingOrder
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingOrder]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) UpdateOrder(
	ctx context.Context,
	request *junctiongo.UpdateOrderBody,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.PostOrderResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.PostOrderResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPatch,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.PostOrderResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) CreateOrder(
	ctx context.Context,
	request *junctiongo.CreateOrderRequestCompatible,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.PostOrderResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/order"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	if request.IdempotencyKey != nil {
		headers.Add("X-Idempotency-Key", *request.IdempotencyKey)
	}
	if request.IdempotencyError != nil {
		headers.Add("X-Idempotency-Error", *request.IdempotencyError)
	}
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.PostOrderResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.PostOrderResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) ImportOrder(
	ctx context.Context,
	request *junctiongo.ImportOrderBody,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.PostOrderResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/order/import"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.PostOrderResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.PostOrderResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) CancelOrder(
	ctx context.Context,
	request *junctiongo.CancelOrderLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.PostOrderResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/cancel",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.PostOrderResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.PostOrderResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) SimulateOrderProcess(
	ctx context.Context,
	request *junctiongo.SimulateOrderProcessLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[any], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/test",
		request.OrderId,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response any
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[any]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) UpdateOnSiteCollectionOrderDrawCompleted(
	ctx context.Context,
	request *junctiongo.UpdateOnSiteCollectionOrderDrawCompletedLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.PostOrderResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/order/%v/draw_completed",
		request.OrderId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.PostOrderResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPatch,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.PostOrderResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) ListUnmatchedResultTestCases(
	ctx context.Context,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ListUnmatchedResultTestCasesResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/unmatched_result_test/case"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.ListUnmatchedResultTestCasesResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ListUnmatchedResultTestCasesResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) CreateUnmatchedResultTest(
	ctx context.Context,
	request *junctiongo.CreateUnmatchedResultTestBody,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.CreateUnmatchedResultTestResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/unmatched_result_test"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("X-Idempotency-Key", request.IdempotencyKey)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.CreateUnmatchedResultTestResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.CreateUnmatchedResultTestResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetUnmatchedResultTest(
	ctx context.Context,
	request *junctiongo.GetUnmatchedResultTestLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.GetUnmatchedResultTestResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/unmatched_result_test/%v",
		request.RunId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.GetUnmatchedResultTestResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.GetUnmatchedResultTestResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) ListUnmatchedResults(
	ctx context.Context,
	request *junctiongo.ListUnmatchedResultsLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ListUnmatchedResultResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/unmatched_result"
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.ListUnmatchedResultResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ListUnmatchedResultResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) GetUnmatchedResult(
	ctx context.Context,
	request *junctiongo.GetUnmatchedResultLabTestsRequest,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.GetUnmatchedResultResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/unmatched_result/%v",
		request.RawResultId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *junctiongo.GetUnmatchedResultResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.GetUnmatchedResultResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) AcceptUnmatchedResult(
	ctx context.Context,
	request *junctiongo.AcceptUnmatchedResultBody,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ClientFacingOrder], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/unmatched_result/%v/accept",
		request.RawResultId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ClientFacingOrder
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ClientFacingOrder]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) ResolveUnmatchedResult(
	ctx context.Context,
	request *junctiongo.ResolveUnmatchedResultBody,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.UnmatchedResult], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/v3/unmatched_result/%v/resolve",
		request.RawResultId,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.UnmatchedResult
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.UnmatchedResult]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) ValidateIcdCodes(
	ctx context.Context,
	request *junctiongo.ValidateIcdCodesBody,
	opts ...option.RequestOption,
) (*core.Response[*junctiongo.ValidateIcdCodesResponse], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://api.us.junction.com",
	)
	endpointURL := baseURL + "/v3/insurance/validate_icd_codes"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *junctiongo.ValidateIcdCodesResponse
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			DisableRetries:  options.DisableRetries,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(junctiongo.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*junctiongo.ValidateIcdCodesResponse]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}
